...
A row of labels only says you can switch. First decide **which selection model this row is**.
...
| Chevron steps | Arrows bite; whichever step you land on, **before it is done, it is current, after it is todo** | Current is filled, done is quieter, todo is light. Not sibling views |
...
- **Chevron is not sibling tabs.** Done means “before the current step,” not four exclusive views. Sibling sections should not bite with arrows.
...
| Members / roles | Tabs float above the panel with a seam | The current tab matches the panel, drops its bottom edge, and joins it |
...
Geometry lives in DOM-free modules: `textIndicator` (the text span for linear, the whole item for segmented), `indicatorTransition`, `stepKind`, `cardPanelRadius`, `folderLayer`, `bevelInset`.
