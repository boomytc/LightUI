...
const LAB = process.env.LAB_URL ?? "http://127.0.0.1:5173";
...
function dests(study, name, locale) {
  ...
  return [
    path.join(repoRoot, "studies", study, "references", name),
...
try {
  ...
  if (!probe?.ok) {
    throw new Error(`Lab is not reachable at ${LAB}. Start it with: make dev`);
  }
