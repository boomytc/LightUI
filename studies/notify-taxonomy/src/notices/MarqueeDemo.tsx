...
export function MarqueeDemo({ state }: { state?: string } = {}) {
  ...
  return (
    <Frame
      ...
      bar={
        visible ? (
          <div className="notify-marquee flex h-8 shrink-0 items-center gap-2 bg-fg px-2 text-surface">
            ...
            <div className="h-8 min-w-0 flex-1 overflow-hidden">
              <div className="notify-marquee-track flex w-max items-center gap-10 whitespace-nowrap text-[12px] leading-8">
                {loop.map((item, i) => (
                  <span key={`${item}-${i}`}>{item}</span>
                ))}
              </div>
            </div>
            <span className="shrink-0 text-[10px] text-surface/55">
              {locale === "en" ? "Hover pauses" : "悬停暂停"}
            </span>
          </div>
        ) : null
      }
