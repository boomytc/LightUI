  ...
  return (
    <Frame title={locale === "en" ? "Portfolio" : "作品集"}>
      <div className="h-full overflow-x-hidden overflow-y-auto">
        <HeroWash />
        <div className="sticky top-3 z-20 -mt-11 flex justify-center px-3">
          <nav
            aria-label={locale === "en" ? "Primary" : "主导航"}
            className="flex max-w-full items-center gap-1 overflow-x-auto rounded-full bg-surface px-2 py-1.5 shadow-card"
          >
            <span className="shrink-0 px-2.5 text-[13px] font-medium">{locale === "en" ? "Studio" : "工作室"}</span>
