...
def load_backend() -> tuple[str, str]:
    ...
    if not base:
        raise SystemExit(
            "Missing MODELBEST_BASE_URL, or modelbest_base_url in tools/study-films/config.local.yaml"
        )
    if not key:
        raise SystemExit(
            "Missing MODELBEST_API_KEY, or modelbest_api_key in tools/study-films/config.local.yaml"
        )
    return base, key
...
def main() -> None:
    ...
    parser.add_argument("--film", choices=["intent-cascade", "dropdown-taxonomy", "all"], default="all")
    ...
    films = list(narration["films"]) if args.film == "all" else [args.film]
