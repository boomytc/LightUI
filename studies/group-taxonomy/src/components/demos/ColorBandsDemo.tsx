...
const BANDS = [
  {
    category: "团队沟通",
    style: "bg-accent-soft/50 dark:bg-accent-soft/10",
    items: [
  ...
  {
    category: "项目进度",
    style: "bg-intent-soft/50 dark:bg-intent-soft/10",
    items: [
...
function Grouped() {
  return (
    <div className="bg-surface">
      <h3 className="px-5 pt-5 text-lg font-semibold tracking-tight text-fg sm:px-6 sm:pt-6">
        团队协作
      </h3>
      <div className="mt-4 flex flex-col">
        {BANDS.map((band) => (
          <div key={band.category} className={`px-5 py-5 sm:px-6 ${band.style}`}>
            <p className="mb-3 text-[0.7rem] font-semibold tracking-wider text-fg uppercase">
              {band.category}
            </p>
